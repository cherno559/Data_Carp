...
def extraer_plantilla_river(ruta_excel_str):
    try:
        xl = pd.ExcelFile(ruta_excel_str)
        filas = []
...
def simular_montecarlo(lam_r, lam_v):
    ...
    return {
        "prob_victoria": float(np.mean(gr > gv)), "prob_empate": float(np.mean(gr == gv)),
...
def obtener_tabla_goleadores(titulares, df_plantilla, lam_r):
    ...
    def amenaza_base(pos): return 0.05 if "Delantero" in pos else 0.03 if "Mediocampista" in pos else 0.015
    def peso_pos(pos): return 1.3 if "Delantero" in pos else 1.0 if "Mediocampista" in pos else 0.7
    df_xi["amenaza"] = (df_xi["xG_p90"] + df_xi["Posicion"].apply(amenaza_base)) * df_xi["Posicion"].apply(peso_pos)
    total = df_xi["amenaza"].sum() or 1
...
def fig_heatmap(sim, rival, style_fn=None):
    ...
    fig.update_layout(
        title=dict(text=f"MAPA DE MARCADORES — River vs {rival}"),
        height=500, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(249,250,251,1)",
        xaxis=dict(title="GOLES RIVER PLATE", gridcolor="#E5E7EB"),
        yaxis=dict(title=f"GOLES {rival.upper()}", gridcolor="#E5E7EB")
    )
    return fig
...
# ─────────────────────────────────────────────────────────────────────────────
def render_predictor(ruta_excel, apply_plotly_style_fn=None):
    st.markdown("""<style>
    ...
    if df_plantilla.empty:
        st.error("❌ Error: No se pudo leer la plantilla. Asegurate de tener datos cargados en el Excel de esta temporada.")
        return

    c1, c2 = st.columns([2, 1])
    with c1: rival = st.selectbox("🆚 Seleccioná el Rival", sorted([e for e in EQUIPOS_PRIMERA_2026 if e != "River"]))
    with c2: es_local = st.radio("📍 Localía", ["Monumental 🏟️", "Visitante ✈️"]) == "Monumental 🏟️"
    ...
    if st.button("🚀 SIMULAR PARTIDO", use_container_width=True, type="primary"):
        ...
        with t1:
            fig = fig_heatmap(sim, rival)
            if apply_plotly_style_fn:
                try: apply_plotly_style_fn(fig)
                except: pass
            st.plotly_chart(fig, use_container_width=True)
        with t2: st.table(obtener_tabla_goleadores(titulares, df_plantilla, lr))
        with t3: st.dataframe(df_plantilla[df_plantilla["Jugador"].isin(titulares)], use_container_width=True)
