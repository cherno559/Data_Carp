    ...
    try:
        ...
        for hoja in xl.sheet_names:
            ...
            if 'Jugador' in df.columns and 'Nota SofaScore' in df.columns:
                ...
                # Normalización de Posición (Crucial para que no rompa Plotly)
                ...
                cols_num = {
                    ...
                    'Efectividad Pases': 'Efectividad Pases'
                }
                
                for col_excel, col_norm in cols_num.items():
                    if col_excel in df.columns:
                        df[col_norm] = pd.to_numeric(df[col_excel], errors='coerce').fillna(0)
                    elif col_norm not in df.columns:
                        df[col_norm] = 0
...
if menu == "Resumen General":
    ...
    st.dataframe(
        ...
        column_config={
            "Tendencia": st.column_config.BarChartColumn("Forma (Últ. 5)", y_min=5, y_max=10)
        },
    ...
    if not df_map.empty:
        ...
        if not df_p90.empty:
            ...
            st.warning("No hay jugadores que cumplan el filtro de minutos mínimos.")
    else:
        st.warning("No hay datos de minutos disponibles.")
